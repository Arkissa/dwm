...
# 更新状态栏
refresh() {
    _blues=''; _cpu=''; _mem=''; _date=''; _vol=''; _bat=''; _light='';             # 重置所有模块的状态为空
    source $DWM/statusbar/temp                                           # 从 temp 文件中读取模块的状态
    xsetroot -name "$_cpu$_mem$_vol$_blues$_bat$_light$_date"                   # 更新状态栏
}

# 启动定时更新状态栏 不用的package有不同的刷新周期 注意不要重复启动该func
cron() {
    while true; do update cpu mem;        sleep 5;  done &              # 每隔20s更新cpu 内存
    while true; do update vol blues;      sleep 5;  done &              # 每隔60s更新音量 图标
    while true; do update bat light;      sleep 5;  done &              # 每隔300s更新电池
    while true; do update date; refresh;  sleep 1;  done &              # 每隔5s更新时间并更新状态栏
}
...
case $1 in
    ...
    updateall|check) update cpu mem vol blues bat light date; refresh ;;
